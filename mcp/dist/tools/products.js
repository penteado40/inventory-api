...
export function registerProductTools(server) {
    server.tool("list_products", "List all products for the current store. Requires API_TOKEN and API_STORE_ID env vars. Optional filters: status, category, search by name/code.", {
    ...
    server.tool("create_product", "Create a new product for the current store. Requires admin or operator role and API_STORE_ID env var. Required: name, category, locationId, buyer, cost.", {
    ...
    }, async ({ name, category, locationId, buyer, cost, arrivedAt, status }) => {
    ...
    server.tool("get_product_by_id", "Get a product by its ID. Requires API_TOKEN and API_STORE_ID env vars.", {
        id: z.number().int().positive().describe("Product ID"),
    }, async ({ id }) => {
        const result = await apiRequest(`/products/${id}`);
        return toContent(result);
    });
    server.tool("update_product", "Update a product's fields. Requires admin or operator role and API_STORE_ID env var. When changing status to 'em_estoque', arrivedAt is required.", {
        id: z.number().int().positive().describe("Product ID"),
    ...
    }, async ({ id, name, category, locationId, buyer, cost, arrivedAt, status }) => {
    ...
    server.tool("delete_product", "Delete a product. Fails if it has linked movements. Requires admin role and API_STORE_ID env var.", {
        id: z.number().int().positive().describe("Product ID"),
    }, async ({ id }) => {
