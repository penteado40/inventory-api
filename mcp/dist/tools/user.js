...
export function registerUserTools(server) {
    server.tool("list_users", "List all users. Requires admin role. Optional filters: search by name/email, filter by role or active status.", {
    ...
    server.tool("create_user", "Create a new user. Requires admin role. Required: name, email, password, role. Optional: storeId, phone.", {
    ...
    }, async ({ name, email, password, role, storeId, phone }) => {
    ...
    server.tool("get_user_by_id", "Get a user by their ID. Requires admin role.", {
        id: z.number().int().positive().describe("User ID"),
    }, async ({ id }) => {
        const result = await apiRequest(`/users/${id}`);
        return toContent(result);
    });
    server.tool("update_user", "Update a user's name and/or phone. Requires admin role. Only name and phone can be updated here.", {
        id: z.number().int().positive().describe("User ID"),
    ...
    server.tool("reset_user_password", "Reset a user's password. Requires admin role.", {
        id: z.number().int().positive().describe("User ID"),
    ...
    server.tool("change_user_role", "Change a user's role. Requires admin role.", {
        id: z.number().int().positive().describe("User ID"),
    ...
    server.tool("change_user_store", "Assign or remove a user's store association. Requires admin role. Pass storeId: null to remove the link.", {
        id: z.number().int().positive().describe("User ID"),
    ...
    server.tool("delete_user", "Permanently delete a user. Requires admin role.", {
        id: z.number().int().positive().describe("User ID"),
    }, async ({ id }) => {
