...
export function registerLocationTypeTools(server: McpServer): void {
  server.tool(
    "list_location_types",
    "List all location types for the current store. Requires API_TOKEN and API_STORE_ID env vars. Optional: search by name.",
    {
  ...
  server.tool(
    ...
    "Create a new location type for the current store. Requires admin role and API_STORE_ID env var. Required: name.",
    {
  ...
  server.tool(
    ...
    "Get a location type by its ID. Requires API_TOKEN and API_STORE_ID env vars.",
    {
  ...
  server.tool(
    ...
    "Update a location type's name. Requires admin role and API_STORE_ID env var.",
    {
      id: z.number().int().positive().describe("Location type ID"),
  ...
  server.tool(
    ...
    "Delete a location type. Fails if any locations are using this type. Requires admin role and API_STORE_ID env var.",
    {
      id: z.number().int().positive().describe("Location type ID"),
    },
    async ({ id }: { id: number }) => {
