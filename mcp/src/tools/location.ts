...
export function registerLocationTools(server: McpServer): void {
  server.tool(
    "list_locations",
    "List all locations for the current store, including nested children. Requires API_TOKEN and API_STORE_ID env vars. Optional filters: locationTypeId, parentId, search by displayName.",
    {
    ...
    async ({ locationTypeId, parentId, q }: { locationTypeId?: number; parentId?: number; q?: string }) => {
      const params = new URLSearchParams();
      if (locationTypeId !== undefined) params.set("locationTypeId", String(locationTypeId));
  ...
  server.tool(
    "create_location",
    "Create a new location for the current store. Requires admin role and API_STORE_ID env var. Required: number, locationTypeId. Optional: parentId.",
    {
    ...
    async ({ number, locationTypeId, parentId }: { number: number; locationTypeId: number; parentId?: number | null }) => {
  ...
  server.tool(
    "get_location_by_id",
    "Get a location by its ID, including its direct children. Requires API_TOKEN and API_STORE_ID env vars.",
    {
  ...
  server.tool(
    "update_location",
    "Update a location's fields. Requires admin role and API_STORE_ID env var. Validates circular references in parentId.",
    {
      id: z.number().int().positive().describe("Location ID"),
    ...
    async ({ id, number, locationTypeId, parentId }: { id: number; number?: number; locationTypeId?: number; parentId?: number | null }) => {
  ...
  server.tool(
    "delete_location",
    "Delete a location. Fails if it has children or allocated products. Requires admin role and API_STORE_ID env var.",
    {
      id: z.number().int().positive().describe("Location ID"),
    },
    async ({ id }: { id: number }) => {
