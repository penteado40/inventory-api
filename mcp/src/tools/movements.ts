...
export function registerMovementTools(server: McpServer): void {
  server.tool(
    "list_movements",
    "List all movements for the current store. Requires API_TOKEN and API_STORE_ID env vars. Optional filters: status, movementTypeId, search by code/requester.",
    {
    ...
    async ({ status, movementTypeId, q }: { status?: "preparando" | "enviado" | "entregue"; movementTypeId?: number; q?: string }) => {
      const params = new URLSearchParams();
      if (status) params.set("status", status);
      if (movementTypeId !== undefined) params.set("movementTypeId", String(movementTypeId));
      if (q) params.set("q", q);
  ...
  server.tool(
    "create_movement",
    "Create a new movement for the current store. Requires admin or operator role and API_STORE_ID env var. Required: movementTypeId, requester, movedAt.",
    {
    ...
    async ({ movementTypeId, requester, movedAt, notes }: { movementTypeId: number; requester: string; movedAt: string; notes?: string }) => {
  ...
  server.tool(
    "get_movement_by_id",
    "Get a movement by its ID including items and payments. Requires API_TOKEN and API_STORE_ID env vars.",
    {
  ...
  server.tool(
    "update_movement",
    "Update a movement's fields. Requires admin or operator role and API_STORE_ID env var.",
    {
      id: z.number().int().positive().describe("Movement ID"),
    ...
    async ({ id, requester, movedAt, status, notes }: { id: number; requester?: string; movedAt?: string; status?: "preparando" | "enviado" | "entregue"; notes?: string }) => {
  ...
  server.tool(
    "delete_movement",
    "Cancel a movement and revert its items. Requires admin role and API_STORE_ID env var.",
    {
      id: z.number().int().positive().describe("Movement ID"),
    },
    async ({ id }: { id: number }) => {
  ...
  server.tool(
    "add_movement_item",
    "Add an item to a movement. Requires admin or operator role and API_STORE_ID env var. Required: productId, quantity, productValue, paidValue.",
    {
      id: z.number().int().positive().describe("Movement ID"),
    ...
    async ({ id, productId, quantity, productValue, paidValue }: { id: number; productId: number; quantity: number; productValue: number; paidValue: number }) => {
  ...
  server.tool(
    "update_movement_item",
    "Update an item in a movement. Requires admin or operator role and API_STORE_ID env var. paidValue must be <= productValue.",
    {
      id: z.number().int().positive().describe("Movement ID"),
    ...
    async ({ id, itemId, quantity, productValue, paidValue }: { id: number; itemId: number; quantity?: number; productValue?: number; paidValue?: number }) => {
  ...
  server.tool(
    "delete_movement_item",
    "Remove an item from a movement. Requires admin role and API_STORE_ID env var.",
    {
      id: z.number().int().positive().describe("Movement ID"),
      itemId: z.number().int().positive().describe("Item ID"),
    },
    async ({ id, itemId }: { id: number; itemId: number }) => {
  ...
  server.tool(
    "add_movement_payment",
    "Register a payment for a movement. Requires admin or operator role and API_STORE_ID env var. Required: amount, paidAt.",
    {
      id: z.number().int().positive().describe("Movement ID"),
    ...
    async ({ id, amount, paidAt, notes }: { id: number; amount: number; paidAt: string; notes?: string }) => {
