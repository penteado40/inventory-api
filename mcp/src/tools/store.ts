...
export function registerStoreTools(server: McpServer): void {
  server.tool(
    "list_stores",
    "List all stores. Requires admin role. Optional filters: search by name/slug and filter by active status.",
    {
    ...
    async ({ q, active }: { q?: string; active?: boolean }) => {
  ...
  server.tool(
    "create_store",
    "Create a new store. Requires admin role. Required: name. Optional: address, phone, requireProductCode.",
    {
    ...
    async ({ name, address, phone, requireProductCode }: { name: string; address?: string; phone?: string; requireProductCode?: boolean }) => {
  ...
  server.tool(
    "get_store_by_id",
    "Get a store by its ID. Requires admin role.",
    {
  ...
  server.tool(
    "update_store",
    "Update a store's fields. Requires admin role. Slug cannot be changed. All fields are optional.",
    {
      id: z.number().int().positive().describe("Store ID"),
    ...
    async ({ id, name, address, phone, requireProductCode }: { id: number; name?: string; address?: string; phone?: string; requireProductCode?: boolean }) => {
  ...
  server.tool(
    "delete_store",
    "Soft-delete a store (sets active = false). The record is not removed from the database. Requires admin role.",
    {
      id: z.number().int().positive().describe("Store ID"),
    },
    async ({ id }: { id: number }) => {
